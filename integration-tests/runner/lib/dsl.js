...
class ScenarioBuilder {
  ...
  teleport(x, y, z) {
    this.steps.push(this._action(`teleport ${x},${y},${z}`, (ctx) => ctx.actor.teleport(x, y, z)));
    return this;
  }
  give(item, qty = 1) {
    this.steps.push(this._action(`give ${qty}x ${item}`, (ctx) => ctx.actor.give(item, qty)));
    return this;
  }
  placeRegion(type, x, y, z) {
    this.steps.push(
      this._action(`placeRegion ${type} @ ${x},${y},${z}`, (ctx) =>
        ctx.actor.placeRegion(type, x, y, z)
      )
    );
    return this;
  }
  runCommand(cmd) {
    this.steps.push(this._action(`runCommand ${cmd}`, (ctx) => ctx.actor.runCommand(cmd)));
    return this;
  }
  waitTicks(n) {
    this.steps.push({ kind: 'wait', run: (ctx) => ctx.waitTicks(n) });
    return this;
  }
  ...
  _action(desc, fn) {
    return {
      ...
      run: async (ctx) => {
        if (!ctx.actor || !ctx.actor.available) {
          ctx.expectTrue(
            `action requires actor: ${desc}`,
            false,
            ctx.actor ? ctx.actor.reason : 'no actor'
          );
  ...
  expectRegion(type, x, y, z, world) {
    this.steps.push({
      ...
      run: async (ctx) =>
        ctx.expect(
          `region ${type} exists at ${x},${y},${z}`,
          await ctx.harness.assert.region(type, x, y, z, world)
        ),
    });
    return this;
  }
  expectMoney(player, op, amt) {
    this.steps.push({
      ...
      run: async (ctx) =>
        ctx.expect(
          `economy ${player} ${op} ${amt}`,
          await ctx.harness.assert.economy(player, op, amt)
        ),
    });
    return this;
  }
  expectPermission(player, node, val) {
    this.steps.push({
      ...
      run: async (ctx) =>
        ctx.expect(
          `permission ${node}=${val}`,
          await ctx.harness.assert.permission(player, node, val)
        ),
    });
    return this;
  }
  expectBlock(x, y, z, material, world) {
    this.steps.push({
      ...
      run: async (ctx) =>
        ctx.expect(
          `block ${material} at ${x},${y},${z}`,
          await ctx.harness.assert.block(x, y, z, material, world)
        ),
    });
    return this;
  }
  ...
  expectNoErrors(opts) {
    this._checkNoErrors = true;
    this._noErrorsIgnore = (opts && opts.ignore) || [];
    return this;
  }

  // --- persistence / arrange (harness ops: reset/observe/fixture — never Civs creation) ---
  saveRegions() {
    this.steps.push({ kind: 'op', run: (ctx) => ctx.harness.region.save() });
    return this;
  }
  reloadRegions() {
    this.steps.push({
      ...
      run: async (ctx) => {
        ...
        ctx.expectTrue(
          'reload loaded regions from disk',
          Number.parseInt(r.regions, 10) > 0,
          r._raw
        );
      },
    });
  ...
  arrangeMoney(player, amt) {
    this.steps.push({ kind: 'op', run: (ctx) => ctx.harness.money.set(player, amt) });
    return this;
  }
  arrangeBlock(x, y, z, material) {
    this.steps.push({ kind: 'op', run: (ctx) => ctx.harness.block.set(x, y, z, material) });
    return this;
  }
  wait(ms) {
    this.steps.push({ kind: 'wait', run: () => new Promise((r) => setTimeout(r, ms)) });
    return this;
  }
  /** Escape hatch for a custom step: fn(ctx). */
  step(desc, fn) {
    this.steps.push({ kind: 'op', desc, run: fn });
    return this;
  }

  // --- teardown (harness reset; runs even on failure) ---
  resetRegion(x, y, z, world) {
    this.teardown.push((ctx) => ctx.harness.region.reset(x, y, z, world));
    return this;
  }
  resetBlock(x, y, z, world) {
    this.teardown.push((ctx) => ctx.harness.block.reset(x, y, z, world));
    return this;
  }
  cleanup(fn) {
    ...
    return {
      ...
      async run(ctx) {
        ...
        if (self._checkNoErrors) {
          ...
          await ctx.expectTrue(
            'no server errors during scenario',
            errs.length === 0,
