...
async function createActor(opts) {
  ...
  try {
    ...
    return { available: false, reason: 'mineflayer not installed', async disconnect() {} };
  }
  ...
  return await new Promise((resolve) => {
    ...
    try {
      ...
      return finish({
        available: false,
        reason: 'createBot threw: ' + e.message,
        async disconnect() {},
      });
    }
    const timeout = setTimeout(
      () =>
        finish({
          available: false,
          reason: 'no spawn within 20s (protocol/plugin incompatibility)',
          async disconnect() {
            try {
              bot.quit();
            } catch (_) {}
    ...
    bot.once('spawn', () => {
      ...
      finish({
        ...
        async command(cmd) {
          bot.chat('/' + cmd.replace(/^\//, ''));
        },
        position() {
          return bot.entity && bot.entity.position;
        },
        heldItemName() {
          return bot.heldItem ? bot.heldItem.name : null;
        },
        inventoryNames() {
          return bot.inventory.items().map((i) => i.name);
        },
        async disconnect() {
          try {
            bot.quit();
    ...
    bot.on('error', (e) =>
      finish({ available: false, reason: 'error: ' + e.message, async disconnect() {} })
    );
    process.once('uncaughtException', (e) =>
      finish({ available: false, reason: 'uncaught: ' + e.message, async disconnect() {} })
    );
  });
}
