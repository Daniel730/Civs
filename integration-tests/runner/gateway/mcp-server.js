...
const REPORTS_DIR = process.env.GATEWAY_REPORTS_DIR || path.join(__dirname, '..', 'reports');
...
function envelope(status, action, payload = {}, reason = null) {
  return {
    ...
    ...payload,
    _lesson:
      'success flags from performCommand are insufficient; prefer harness.json.success + state observe',
  };
}
...
server.tool(
  ...
  {},
  async () =>
    withToolLog('minecraft_observe', {}, async () => {
...
server.tool(
  ...
  async ({ x, y, z, timeout_ms, arrive }) =>
    withToolLog('minecraft_move_to', { x, y, z, timeout_ms, arrive }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
      const r = await harness.cap.moveTo(
        cfg.actorName,
        x,
        y,
        z,
...
server.tool(
  ...
  async ({ yaw, pitch }) =>
    withToolLog('minecraft_look', { yaw, pitch }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
...
server.tool(
  ...
  async ({ x, y, z }) =>
    withToolLog('minecraft_break_block', { x, y, z }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
      return textResult(
        fromCap('break_block', await harness.cap.breakBlock(cfg.actorName, x, y, z))
      );
    })
);
...
server.tool(
  ...
  async ({ x, y, z, material }) =>
    withToolLog('minecraft_place_block', { x, y, z, material }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
      return textResult(
        fromCap('place_block', await harness.cap.placeBlock(cfg.actorName, x, y, z, material))
      );
    })
);
...
server.tool(
  ...
  async ({ entity_type }) =>
    withToolLog('minecraft_attack_nearest', { entity_type }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
      return textResult(
        fromCap('attack', await harness.cap.attackNearest(cfg.actorName, entity_type))
      );
    })
);
...
server.tool(
  ...
  {},
  async () =>
    withToolLog('minecraft_rpg_observe', {}, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
...
server.tool(
  ...
  { quest_id: z.string() },
  async ({ quest_id }) =>
    withToolLog('minecraft_rpg_accept', { quest_id }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
      return textResult(
        fromCap('rpg_accept', await harness.cap.rpgAccept(cfg.actorName, quest_id))
      );
    })
);
...
server.tool(
  ...
  { quest_id: z.string() },
  async ({ quest_id }) =>
    withToolLog('minecraft_rpg_abandon', { quest_id }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
      return textResult(
        fromCap('rpg_abandon', await harness.cap.rpgAbandon(cfg.actorName, quest_id))
      );
    })
);
...
server.tool(
  ...
  { x: z.number(), y: z.number(), z: z.number() },
  async ({ x, y, z }) =>
    withToolLog('minecraft_teleport', { x, y, z }, async () => {
      const blocked = await ensureSession();
      if (blocked) return textResult(blocked);
