...
class ScenarioContext {
  ...
  expect(name, assertResult) {
    return withSpan('scenario.assertion', {
      ...
      setSpanAttrs(span, { 'assertion.message': r.message ? String(r.message).slice(0, 200) : undefined });
      return ok;
    });
  }
  ...
  async withRetry(name, attempts, fn) {
    ...
    return withSpan('scenario.retry', {
      'scenario.name': this.scenarioName || undefined,
      'scenario.step': name,
      'retry.max': max,
    }, async (span) => {
